...
public class ProductRepository extends BaseRepository<ProductEntity> implements ProductRepositoryInterface {
	...
	public ProductEntity byLookupCode(String lookupCode) {
		return this.firstOrDefaultWhere(
				new WhereContainer(
						(new WhereClause()).postgreFunction(PostgreFunctionType.LOWER).table(this.primaryTable)
								.fieldName(ProductFieldNames.LOOKUP_CODE).comparison(SQLComparisonType.EQUALS)),
				(ps) -> {
					try {

...
public class AdditionalJoinCriteria
{
	...
	public String toString() {
		...
		StringBuilder sb = new StringBuilder(SPACE).append(conditional.getLabel()).append(SPACE);

		if (joinOnApplyFunction != PostgreFunctionType.NONE) {
			sb.append(joinOnApplyFunction.getLabel()).append(OPEN_FUNCTION);
		}

		sb.append(joinOnTableName).append(TABLE_FIELD_SEPARATOR).append(joinOnFieldName);

		if (joinOnApplyFunction != PostgreFunctionType.NONE) {
			sb.append(CLOSE_FUNCTION);
		}

		sb.append(SPACE).append(comparison.getLabel()).append(SPACE);
		...
		if (this.equivalencyFunction != PostgreEquivalencyType.NONE) {
			...
			sb.append(externalTableName).append(TABLE_FIELD_SEPARATOR).append(externalTableFieldName);
		}
